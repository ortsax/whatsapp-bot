...
func init() {
	// Background TTL cleanup every 10 minutes.
	go func() {
...
func antiDeleteHook(client *whatsmeow.Client, evt *events.Message) {
	// Always cache incoming messages regardless of toggle state.
	cacheMessage(evt)
	...
	// Ignore the bot itself deleting.
	if evt.Info.IsFromMe {
		return
	}

	// Ignore owner and sudoers (check both primary JID and alt JID for LID/phone).
	...
	// For groups: also ignore group admins.
	if isGroup {
