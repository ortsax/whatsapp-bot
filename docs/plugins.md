...
Place this file anywhere inside `plugins/` and it will be compiled in automatically.
...
To add translated strings for your plugin, extend the `Strings` struct and each language block in `plugins/i18n.go`, then access them via:
...
```bash
# Build
make build        # produces alphonse.exe on Windows, alphonse elsewhere
